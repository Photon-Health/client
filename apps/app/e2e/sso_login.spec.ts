...
test('SSO login flow and logout loop regression', async ({ page }) => {
  ...
  expectSingleLoginRedirect(authorizeRequests, 'fakecustomer');
  ...
  expect(page.url()).toContain('connection=fakecustomer');

  await expect(page.getByRole('heading', { name: 'Prescriptions' })).toBeVisible({
    timeout: 60_000
  });

  expectSingleLogoutRedirect(navigationUrls, 'fakecustomer');
  await expectPhotonHomePageVisible(page);
});
