...
function sourceRank(offer: OfferBundleView): number {
  const index = CLIENT_SOURCE_PRIORITY.indexOf(offer.source ?? '');
  return index === -1 ? CLIENT_SOURCE_PRIORITY.length : index;
}

export function isDeliveryOffer(offer: OfferBundleView): boolean {
...
export interface OfferPlacement {
  aboveFold: OfferBundleView[]; // all promoted offers, source-priority ordered
  inTab: OfferBundleView[]; // the rest fall into their respective tabs
}

export function selectOfferPlacement(allOffers: OfferBundleView[] | undefined): OfferPlacement {
  return {
    aboveFold: (allOffers ?? [])
